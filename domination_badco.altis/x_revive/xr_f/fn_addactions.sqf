...
if (_unit getVariable ["xr_ReviveAction", -9999] == -9999) then {
	private _id = [
		...
		/* 4 condition to show */			"_this distance2D _target <= 3.5 && {_target getVariable 'xr_pluncon' && {!(_this getVariable 'xr_pisinaction') && {!(_target getVariable 'xr_dragged')}}} && {cursorObject == _target}",
		/* 5 condition for action */		"((_this distance2D _target) <= 3.5) && {cursorObject == _target} && {!(_this getVariable 'xr_pluncon')} && {_target getVariable 'xr_pluncon'} && {!(_target getVariable 'xr_dragged')}",
