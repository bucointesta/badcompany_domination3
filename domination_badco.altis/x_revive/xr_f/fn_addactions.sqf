...
if (_unit getVariable ["xr_ReviveAction", -9999] == -9999) then {
	private _id = [
		...
		/* 5 condition for action */		"((_this distance2D _target) <= 3.5) && {cursorTarget == _target} && {!(_this getVariable 'xr_pluncon')} && {_target getVariable 'xr_pluncon'} && {!(_target getVariable 'xr_dragged')}",
