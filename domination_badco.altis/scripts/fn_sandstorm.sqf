...
while {true} do {	
	if (rain isEqualTo 0) then {	
        ...
        private _relPos = [-((_velocity select 1) * (__DURATION / 2)), 0, -6];
        ...
        _ps setParticleParams [["\A3\data_f\ParticleEffects\Universal\Universal", 16, 12, 8, 0], "", "Billboard", 1, __DURATION, _relPos, _velocity, 1, 1.275, 1, 0, [5], [_color + [0], _color + [0.2  + random 0.02], _color + [0]], [1000], 1, 0, "", "", _obj];
