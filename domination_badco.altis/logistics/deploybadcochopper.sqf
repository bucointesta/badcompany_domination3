...
if ((isNil "d_chopper_10") || {!alive d_chopper_10}) exitWith {
  hint "UH-1Y not available";
};

if (d_chopper_10 call d_fnc_isVecLocked) then {
  ...
  _attachPos = destroyer_badco worldToModel (getPosATL d_chopper_10);
  _attachPos set [2, 11];
  d_chopper_10 attachTo [destroyer_badco, _attachPos];
  [d_chopper_10, 180] remoteExecCall ["setDir", d_chopper_10];
