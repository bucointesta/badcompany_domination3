...
if (!isNull _killer && {isPlayer _killer && {vehicle _killer != vehicle _killed}}) then {
	...
	if (!_killedfriendly) then {
		...
		format [localize ["STR_DOM_MISSIONSTRING_515", "STR_DOM_MISSIONSTRING_514"] select (d_WithRevive == 0), _namek, _namep, "WEST", d_tt_points select 8] remoteExecCall ["sideChat", [0, -2] select isDedicated];
	} else {
